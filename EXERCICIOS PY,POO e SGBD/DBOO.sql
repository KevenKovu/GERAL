...
select c.codigo as "Código",
	c.nome as "Nome",
    f.cpf as "CPF/CNPJ" , null as "Tipo Sociedade (Se PJ)"
from Cliente c, PessoaFisica f, PessoaJuridica j
where c.codigo = f.codigo 
union 
select c.codigo, c.nome,f.cpf , j.sociedade
from Cliente c, PessoaJuridica j
where c.codigo = j.codigo;

select *
from Cliente c, PessoaJuridica j
where c.codigo = f.codigo  and
